...
def reset_timer():
    ...
    canvas.itemconfig(time_limit, text="00:00")
...
def start_timer():
    ...
    work_min_break = SHORT_BREAK_MIN * 60
    ...
    if reps % 2 != 0:
        ...
        count_down(work_min_break)
        timer_label.config(text="Break", bg=YELLOW, fg=PINK, font=(FONT_NAME, 35, "bold"))
# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    count_min = math.floor(count / 60)
    count_sec = count % 60
    sec = round(count_sec, 1)
    if count_sec < 10:
        count_sec = f"0{count_sec}"

    canvas.itemconfig(time_limit, text=f"{count_min}:{count_sec}")
    print(count)
    if count > 0:
        global timer
...
start_btn = Button(text="Start", bg="white", border=0.5, command=start_timer)
start_btn.grid(column=0, row=3)

reset_btn = Button(text="Reset", bg="white", border=0.5, command=reset_timer)
...
time_limit = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
